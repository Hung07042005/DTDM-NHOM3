...
function selImport(el) {
    document.querySelectorAll('.import-src').forEach(s => { s.classList.remove('sel'); const tick = s.querySelector('[style*="background:var(--sky)"]'); if (tick) tick.remove() });
...
function toast(msg) {
    ...
    const icons = { '✅': true, '❌': true, '⚠️': true, '📋': true, '🔗': true, '💬': true, '✉️': true, '📁': true, '📊': true, '📥': true, '👋': true, '📸': true, '📂': true, '📎': true, '🌐': true, '📱': true, '🎨': true, '🔌': true, '📅': true, '⬆': true, '📨': true, '👤': true, '🗑': true };
